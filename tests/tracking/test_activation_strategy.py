...
def test_stillness_no_previous_returns_false():
    ...
    assert strategy.is_active(make_hand(mcp=(50, 50)), None) is False


def test_stillness_hand_not_moved():
    strategy = StillnessDwellStrategy(velocity_threshold_px_per_frame=5.0)
    hand = make_hand(mcp=(50, 50))
    prev = make_hand(mcp=(51, 50))  # distância = 1 px < 5
...
def test_stillness_hand_moved_too_much():
    strategy = StillnessDwellStrategy(velocity_threshold_px_per_frame=5.0)
    hand = make_hand(mcp=(50, 50))
    prev = make_hand(mcp=(60, 50))  # distância = 10 px > 5
...
def test_stillness_exactly_at_threshold():
    ...
    strategy = StillnessDwellStrategy(velocity_threshold_px_per_frame=5.0)
    hand = make_hand(mcp=(50, 50))
    prev = make_hand(mcp=(55, 50))  # distância = 5.0, não passa o < 5.0
    assert strategy.is_active(hand, prev) is False


def test_stillness_uses_mcp_centroid_not_wrist():
    strategy = StillnessDwellStrategy(velocity_threshold_px_per_frame=5.0)
    hand = make_hand(wrist=(100, 50), mcp=(50, 50))
    prev = make_hand(wrist=(50, 50), mcp=(51, 50))
